...
export class TemporalSliderWindow extends Window {
  ...
  windowCreated() {
    ...
    document
      .getElementById('timeSliderValue')
      .addEventListener('input', this.timeSelection.bind(this), false);
    document
      .getElementById('timeSlider')
      .addEventListener('input', this.timeSelectionSlider.bind(this), false);
  }

  // Call back on new user input with the date selector
  timeSelection() {
    this.currentTime = document
      .getElementById('timeSliderValue')
      .value.toString();

...
export class TemporalProvider {
  ...
  setCityObjectStyle(tileId, cityObjectId, styleName) {
    if (this.tilesManager.isStyleRegistered(styleName)) {
      this.tilesManager.setStyle(
        new CityObjectID(tileId, cityObjectId),
        styleName
      );
    } else {
      console.warn(
  ...
  culling(BT, tileId, tileTransactions) {
    ...
    for (let i = 0; i < BT.featureIds.length; i++) {
      ...
      if (
        ...
        tileTransactions.has(featureId) &&
        tileTransactions.get(featureId)
      ) {
        // ** TRANSACTION CASE
        ...
        if (featureTransactions.asSource) {
          ...
          const transacAsSourceHalfDuration =
            (transacAsSource.endDate - transacAsSource.startDate) / 2;
          if (
            this.currentTime > transacAsSource.startDate &&
            this.currentTime <=
              transacAsSource.startDate + transacAsSourceHalfDuration
          ) {
            hasTransac = true;
            const transactionStyleName = this.getTransactionStyleName(
              transacAsSource,
              ''
            );
        ...
        if (featureTransactions.asDestination) {
          ...
          const transacAsDestHalfDuration =
            (transacAsDest.endDate - transacAsDest.startDate) / 2;
          if (
            this.currentTime >
              transacAsDest.startDate + transacAsDestHalfDuration &&
            this.currentTime <= transacAsDest.endDate
          ) {
            hasTransac = true;
            const transactionStyleName = this.getTransactionStyleName(
              transacAsDest,
        ...
        if (
          this.currentTime + halfVintage >= BT.startDates[i] &&
          this.currentTime < BT.startDates[i]
        ) {
          // ** CREATION
          ...
          this.currentTime - halfVintage < BT.endDates[i] &&
          this.currentTime > BT.endDates[i]
        ) {
          // ** DEMOLITION
  ...
  computeTileState(tileId) {
    ...
    if (
      this.COStyles.has(this.currentTime) &&
      ...
      if (this.tempExtModel.temporalBatchTables.has(tileId)) {
        const tileTemporalBT =
          this.tempExtModel.temporalBatchTables.get(tileId);
        if (!this.COStyles.has(this.currentTime)) {
          this.COStyles.set(this.currentTime, new Map());
        }
        this.COStyles.get(this.currentTime).set(
          tileId,
          this.culling(
            tileTemporalBT,
            tileId,
            this.tempExtModel.transactionsPerTile.get(tileId)
          )
        );
      } else {
